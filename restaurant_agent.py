...
class Reservation(BaseAgent):
    def __init__(self) -> None:
        super().__init__(
            ...
            tools=[update_name, update_phone, to_greeter, disconnect_call],
...
class Takeaway(BaseAgent):
    def __init__(self, menu: str) -> None:
        super().__init__(
            ...
            tools=[to_greeter, disconnect_call],
    ...
    def on_conversation_item_added(ev):
        ...
        if getattr(item, 'type', None) == "message":
            if getattr(item, 'role', None) == "user":
                print(f"\nUser: {item.text_content.strip()}")
            elif getattr(item, 'role', None) == "assistant":
                print(f"Agent: {item.text_content.strip()}")
    session.on("conversation_item_added", on_conversation_item_added)

    # On session close, print the transcript in JSON and conversational format
    def on_close(ev):
        transcript = []
        for item in session.history.items:
            if getattr(item, 'type', None) == "message":
